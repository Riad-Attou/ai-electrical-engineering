...
from utils.motor import BDCMotorParams, SpeedSensorNoise, MotorDataset, generate_dataset
...
class MotorSplit:
    ...
    def save(self, path: str):
        np.savez(path,
                 train_noisy=self.train_noisy,     train_true=self.train_true,
                 train_voltage=self.train_voltage,
                 val_noisy=self.val_noisy,         val_true=self.val_true,
                 val_voltage=self.val_voltage,
                 test_noisy=self.test_noisy,       test_true=self.test_true,
                 test_voltage=self.test_voltage,   dt=self.dt)

    @staticmethod
    def load(path: str) -> "MotorSplit":
        d = np.load(path)
        return MotorSplit(
            train_noisy=d["train_noisy"],     train_true=d["train_true"],
            train_voltage=d["train_voltage"],
            val_noisy=d["val_noisy"],         val_true=d["val_true"],
            val_voltage=d["val_voltage"],
            test_noisy=d["test_noisy"],       test_true=d["test_true"],
            test_voltage=d["test_voltage"],   dt=float(d["dt"]),
        )
...
def _perturb_params(
    ...
    return BDCMotorParams(
        R=p(base.R), L=base.L, Kt=base.Kt, Kb=base.Kb,
        J=p(base.J), B=p(base.B), V_max=base.V_max,
    )
...
def pack_split(
    ...
    return MotorSplit(
        train_noisy=stack(train, "omega_noisy"), train_true=stack(train, "omega_true"),
        train_voltage=stack(train, "voltage"),
        val_noisy=stack(val,   "omega_noisy"),   val_true=stack(val,   "omega_true"),
        val_voltage=stack(val,   "voltage"),
        test_noisy=stack(test,  "omega_noisy"),  test_true=stack(test,  "omega_true"),
