"""
Multi-trajectory dataset utilities for speed-filter training (motor + rod).

Design rationale
----------------
A single long trajectory creates two problems:
...
This module generates N short independent trajectories of the brushed DC motor
with a vertical rod load (the shared team plant), with controlled variability
across motor parameters, sensor noise, and voltage excitation. Train/val/test
are split at the trajectory level so there is zero leakage.

The rod's gravity torque (proportional to sin(theta)) makes the plant nonlinear
and state-dependent — the property a learned filter can exploit and a linear
Kalman filter cannot.
"""
...
class TrajectoryConfig:
    ...
    n_trajectories: int = 210  # total in-distribution trajectories (split below)
    t_end: float = 6.0  # seconds per trajectory
    dt: float = 1e-3
    # Motor params R, J, B are perturbed +/-param_jitter fraction per trajectory
    # to simulate unit-to-unit manufacturing tolerances / model mismatch.
    param_jitter: float = 0.12
    # Per-trajectory Gaussian speed-sensor noise std drawn from this range [rad/s].
    # Sized to ~15% of the rod's speed swing (tens of rad/s), not the bare motor.
    noise_std_range: tuple = (2.0, 4.0)
    # Per-trajectory encoder quantization step [rad/s], 0 = continuous.
    noise_quant_range: tuple = (0.0, 1.0)
    train_frac: float = 0.70
    val_frac: float = 0.15
    # test_frac = 1 - train_frac - val_frac  (~0.15 for defaults)
    # Vertical rod load attached to the motor shaft.
    rod: PendulumParams = field(default_factory=lambda: PendulumParams(m=0.05, l=0.1, g=9.81))
...
# In-distribution excitation families cycled across trajectories.
# "chirp" is deliberately excluded here — it is held out for the OOD set.
...
def _make_noise(config: TrajectoryConfig, rng: np.random.Generator, seed: int) -> SpeedSensorNoise:
    return SpeedSensorNoise(
        std=float(rng.uniform(*config.noise_std_range)),
        quantization=float(rng.uniform(*config.noise_quant_range)),
        # Separate seed so noise is independent from param/excitation RNG.
        rng=np.random.default_rng(seed + 100_000),
    )


# ---------------------------------------------------------------------------
# Public API — in-distribution trajectories
# ---------------------------------------------------------------------------


def generate_multi_trajectory_dataset(
    base_params: BDCMotorParams,
    config: TrajectoryConfig | None = None,
    seed: int = 0,
) -> list[PendulumDataset]:
    """
    Generate N independent motor+rod trajectories with varied motor params,
    noise, and voltage profiles.
    ...
    for k in range(config.n_trajectories):
        ...
        noise = _make_noise(config, traj_rng, traj_seed)

        traj = generate_pendulum_dataset(
            params=params,
            pendulum=config.rod,
            noise=noise,
            dt=config.dt,
...
# ---------------------------------------------------------------------------
# Public API — out-of-distribution (held-out excitation) trajectories
# ---------------------------------------------------------------------------


def generate_chirp_trajectories(
    base_params: BDCMotorParams,
    config: TrajectoryConfig | None = None,
    n: int = 20,
    f0: float = 0.5,
    ...
    """
    Generate motor+rod trajectories driven by a *chirp* (swept-sine) voltage —
    an excitation family never used for training. Used only as an OOD test to
    probe generalisation to unseen inputs.

    The voltage sweeps linearly from f0 to f1 Hz over the trajectory at an
    amplitude near V_max, sign-symmetric so the rod is driven both ways.
    """
    config = config or TrajectoryConfig()
    dt, t_end = config.dt, config.t_end
    steps = int(t_end / dt)
    ...
    for _ in range(n):
        ...
        params = _perturb_params(base_params, config.param_jitter, traj_rng)
        noise = _make_noise(config, traj_rng, traj_seed)
        amp = float(traj_rng.uniform(0.75, 0.95)) * params.V_max
        phase = float(traj_rng.uniform(0, 2 * np.pi))
        voltage = amp * np.sin(2 * np.pi * (f0 * t + 0.5 * k_rate * t**2) + phase)

        motor = BDCMotor(params, pendulum=config.rod)
        ...
        for i in range(steps):
            state = motor.step(dt, float(voltage[i]))
            omega_noisy[i] = noise.measure(state.omega)
            omega_true[i] = state.omega
            theta_true[i] = state.theta
...
def split_trajectories(
    ...
    config: TrajectoryConfig | None = None,
    seed: int = 42,
) -> tuple[list[PendulumDataset], list[PendulumDataset], list[PendulumDataset]]:
    """
...
def pack_split(
    train: list[PendulumDataset],
    val: list[PendulumDataset],
    test: list[PendulumDataset],
    dt: float,
) -> MotorSplit:
    """Stack trajectory lists into (N_traj, T_steps) arrays."""

    def stack(split: list[PendulumDataset], attr: str) -> np.ndarray:
